...
As tabelas `sermon_outline_folders` e `sermon_outline_files` **não existem** no banco de dados. Todas as requisições retornam 404 com a mensagem "Could not find the table 'public.sermon_outline_folders' in the schema cache". O código frontend referencia essas tabelas, mas a migração para criá-las nunca foi executada.

Além disso, o bucket `sermon-outlines` (usado no hook `useEsbocos.tsx`) não existe no storage — apenas `gallery` e `payment-proofs` estão configurados.

## Plano

### 1. Criar migração SQL

Criar as tabelas e o bucket de storage:
...
O frontend (`useEsbocos.tsx`, `Esbocos.tsx`, e a página admin) já está implementado corretamente. O problema é exclusivamente a ausência das tabelas e do bucket no banco.
